...
export class SymbolEx {
    ...
    private getModuleCall(node: CallStatement) {
        ...
        /*--start--此处特殊处理以便于适应module(..., package.seeall)的情况 */ 
        if (argument.type !== "StringLiteral" && argument.type!=="VarargLiteral") {
            return null;
        }
        if (argument.type==="VarargLiteral") {
            argument.type = "StringLiteral";
            argument.value = null;
            const moduleName = path.basename(this.parseUri).slice(0,-4);
            argument.raw  = `\"${moduleName}\"`;
        }
        /*--end-- 此处特殊处理以便于适应module(..., package.seeall)的情况 */ 
    ...
    public parse(uri: string, text: string,
        ...
        // console.log('====103',this.delDocumentSymbol,this.documentModule);
        // 解析成功，更新缓存，否则使用旧的
        ...
        // console.log('====104',this.parseSymList);
        return this.parseSymList;
    }
